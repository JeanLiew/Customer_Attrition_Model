...
git = dbc.NavLink(dbc.CardImg(src="/assets/icons/github.svg", 
                              style={'height':'50px'}),href="https://github.com/JeanLiew")
...
def update_progress(customer_id):
    if customer_id:
        ...
        prog = 100 - countdown
        if 0 < n <= 6: # 6 months
            return [prog, 'success', countdown, 'danger' ]
        elif 6 < n <= 24: #6-24 months
            return [prog, 'success', countdown, 'warning' ]
        elif n > 24: #more 80months
            return [prog, 'success', countdown, 'success' ]  
        else:  #0 month
            return [prog, 'danger', countdown, 'danger' ]
    else:
        return [100,'#fe9d24', 0, 'white'] 
